...
def compute_ylims(df: pd.DataFrame, row_var, col_var, x_cut=0.5):
    lims = []
    df = df[df[NORMALIZED_BUDGET] <= x_cut]
    df.sort_values(by=[row_var, col_var], inplace=True)
    for _, row_df in df.groupby(row_var):
        for _, col_df in row_df.groupby(col_var):
            max_regret = col_df[REGRET].max()
...
def plot_regret(df: pd.DataFrame):
    ...
    df = df.groupby([x, hue, col, row]).mean().reset_index()
    lims = compute_ylims(df, row_var=row, col_var=col)
    palette = create_palette(df)
    df = df.sort_values(by=APPROACH_ORDER)
    g = sns.relplot(data=df, x=x, y=y, hue=hue, col=col, row=row,
